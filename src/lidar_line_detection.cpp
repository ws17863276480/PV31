...
namespace LidarLineDetector {
    ...
    // 版本信息实现
    static const char *versionString = "1.0.4";
    ...
    LidarDetectionResult detectLidarLineWithQuadROI(const cv::Mat& image, const QuadROI& quadRoi, const std::string& sn, const std::string& outputDir)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                logger->warn("四边形ROI超出图像范围，点{}: ({}, {})", i, quadRoi.points[i].x, quadRoi.points[i].y);
                result.status = DetectionResultCode::OUT_OF_ROI;
                // 保存失败图像
                if (!outputDir.empty())
                {
        ...
        if (laserPoints.size() < 100) {
            logger->warn("激光点数量不足: {}", laserPoints.size());
            result.status = DetectionResultCode::NOT_FOUND;
            // 保存失败图像
            if (!outputDir.empty()) {
                cv::Mat resultImage = image.clone();
        ...
        if (rms > 5.0 || length < roiWidth * 0.97) {
            logger->warn("激光点分布不线性或长度不足，RMS: {}, 长度: {}, ROI宽度: {}", rms, length, roiWidth);
            result.status = DetectionResultCode::OUT_OF_ROI;
            // 保存失败图像
            if (!outputDir.empty()) {
                cv::Mat resultImage = image.clone();
        ...
        if (!outputDir.empty())
        {
            ...
            if (!(left_in && right_in)) {
                logger->warn("激光线端点超出四边形ROI边界: left_y={}, right_y={}", pt1_roi.y, pt2_roi.y);
                result.status = DetectionResultCode::OUT_OF_ROI;
                return result;
            }
    ...
    LidarLineResult detect(const cv::Mat &image, const std::string &configPath, const std::string &sn, const std::string &outputDir)
    {
        ...
        if (quadRoiResult == DetectionResultCode::SUCCESS)
        {
            ...
            {
                logger->warn("主检测流程：四边形ROI激光线检测失败，状态: {}", static_cast<int>(detectionResult.status));
        ...
        else
        {
            ...
            {
                logger->error("主检测流程：读取ROI配置失败，状态: {}", static_cast<int>(roiResult));
